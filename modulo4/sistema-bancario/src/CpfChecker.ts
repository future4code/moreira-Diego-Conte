...
export function findingCpf(cpf: string): accountTemplate[] | string {
  const cpfChecked: string | boolean = CpfChecker(cpf);
  let user: accountTemplate[] = [];

  if (cpfChecked === "Please check input: CPF already taken.") {
    user = users.filter((c) => {
      return c.CPF === cpf;
    });
  } else if (cpfChecked === false) {
    return "Please check input: only authentic CPFs are accepted.";
  } else if (cpfChecked === true) {
